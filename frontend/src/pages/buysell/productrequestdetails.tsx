...
import { Attributes, Brand, Category, Model, RamStorageConfig } from '../../constants/models';
import api from '../../utils/network_requests';
import { BASE_URL, UPLOAD_PRODUCT_IMAGE, GET_MODELS } from '../../utils/api';
...
import { ProductInputType, ProductOutputType, useProductMutation, useProductRequestMutation, useSaveImageMutation, useSaveImagePathMutation, useSaveImagePathRequestMutation } from '../../helper/products/use-product';
...
export default function ProductRequestDetails() {
    ...
    const pageTitle = MENU_LABELS.REQUEST_FOR_PART;
    ...
    const { mutateAsync: saveProductForRequest, isLoading } = useProductRequestMutation();
    const { mutateAsync: saveProductRequestImagePaths, isLoading: isSavingImagePaths } = useSaveImagePathRequestMutation();
    const { data:imageData ,mutate: saveProductImages, isLoading: isSavingImages } = useSaveImageMutation();
    ...
    async function onSubmit({ price, description }: ProductInputType) {
        ...
        try {
            ...
            await getApiForProductRequestUpload(pageTitle,productOutput,saveProductForRequest,imageFilePaths,saveProductRequestImagePaths);
    ...
    const handleUpload = async () => {
        ...
        if (firstSelectedImage && secondSelectedImage) {
            ...
            try {
                ...
                const response = await api.post(`${BASE_URL}/${UPLOAD_PRODUCT_IMAGE}/${userId}`, formData); // Replace with your backend URL
                toast.dismiss();
                if (response.data.success === true) {
                    if (response.data.files) {
                        setImageFilePaths(response.data.files.split(","));
                ...
                <form onSubmit={handleSubmit(onSubmit, onInvalid)} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    <Box display={'flex'} sx={{ margin: '2rem', marginBottom: '5rem' }} flexDirection={'column'}>
                        ...
                        {modelsData && modelsData.length > 0 ? (
                            ...
                            <CustomControlledAutoComplete freeSolo={false} control={control} handleValueChange={handleModelChange} itemData={modelsData} isLoading={modelsLoading}
                        ...
                        <CustomControlledAutoComplete freeSolo={false} control={control} handleValueChange={handleRamStorageChange} itemData={filteredRamConfigs} isLoading={ramConfigLoading}
